...
Original file is located at
    https://colab.research.google.com/drive/1EWh0So8f00g2l_NA5JNz0UbwRF7YErPZ
"""
 ...
 #       self.schedule.add(carro5)
    ...
    def getdata(self):
        ...
        for cell in self.grid.coord_iter():
            ...
            for content in cell_content:
              if isinstance(content, Carro):
                  data.append({ 'reference': content.unique_id,
                                'tag': 'carro',
                                'x': content.pos[1],
                                'z': content.pos[0],
                                'luz': None})
...
RANGO DE 0 A 7 (8 FRAMES), LOS PRIMEROS 5 FRAMES SON DE LUZ VERDE LOS OTROS 3 DE LUZ AMARILLA (contadr cilo de luz verde y amarilla)

start_time = time.time()
time_end = time.time() + segundos
#model = Interseccion(width, height, iniBoxes, iniRobots)
model = Interseccion(width, height)
...
class Server(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        #print(positions)
        self._set_response()
        resp = "{\"data\":" + positionsToJSON(data) + "}"
        #print(resp)
